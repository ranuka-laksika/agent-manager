...
type AgentThunderClientRepository interface {
	...
	Upsert(client *models.AgentThunderClient) error

	// FindByAgent returns every binding for the given agent, across all environments.
	FindByAgent(orgName, projectName, agentName string) ([]models.AgentThunderClient, error)

	// Get returns the binding for a specific agent in a specific environment.
	Get(orgName, projectName, agentName, environmentName string) (*models.AgentThunderClient, error)
	...
	FindDue(now time.Time, limit int) ([]models.AgentThunderClient, error)
	...
	ClaimForAttempt(id uuid.UUID) (claimed bool, err error)
	...
	UpdateAfterAttempt(id uuid.UUID, fields AgentThunderAttemptUpdate) error
	...
	MarkClaimed(id uuid.UUID, claimedAt time.Time) (claimed bool, err error)
	...
	UpdateSecretRef(id uuid.UUID, secretRefPath string) error
	...
	ClearClaim(id uuid.UUID) error

	// DeleteByAgent removes every binding for the given agent (used when the agent
	// itself is deleted).
	DeleteByAgent(orgName, projectName, agentName string) error
}
...
func (r *AgentThunderClientRepo) Upsert(c *models.AgentThunderClient) error {
...
func (r *AgentThunderClientRepo) FindByAgent(orgName, projectName, agentName string) ([]models.AgentThunderClient, error) {
	var rows []models.AgentThunderClient
	err := r.db.Where("org_name = ? AND project_name = ? AND agent_name = ?", orgName, projectName, agentName).
		Order("environment_name").Find(&rows).Error
	return rows, err
}

func (r *AgentThunderClientRepo) Get(orgName, projectName, agentName, environmentName string) (*models.AgentThunderClient, error) {
...
func (r *AgentThunderClientRepo) FindDue(now time.Time, limit int) ([]models.AgentThunderClient, error) {
...
func (r *AgentThunderClientRepo) ClaimForAttempt(id uuid.UUID) (bool, error) {
	staleBefore := time.Now().Add(-inProgressStaleThreshold)
	result := r.db.Model(&models.AgentThunderClient{}).
...
func (r *AgentThunderClientRepo) UpdateAfterAttempt(id uuid.UUID, f AgentThunderAttemptUpdate) error {
	updates := map[string]interface{}{
		"status":            f.Status,
	...
	return r.db.Model(&models.AgentThunderClient{}).Where("id = ?", id).Updates(updates).Error
}

func (r *AgentThunderClientRepo) MarkClaimed(id uuid.UUID, claimedAt time.Time) (bool, error) {
	result := r.db.Model(&models.AgentThunderClient{}).
...
func (r *AgentThunderClientRepo) UpdateSecretRef(id uuid.UUID, secretRefPath string) error {
	return r.db.Model(&models.AgentThunderClient{}).Where("id = ?", id).
		Updates(map[string]interface{}{
...
func (r *AgentThunderClientRepo) ClearClaim(id uuid.UUID) error {
	return r.db.Model(&models.AgentThunderClient{}).Where("id = ?", id).
		Update("claimed_at", nil).Error
}

func (r *AgentThunderClientRepo) DeleteByAgent(orgName, projectName, agentName string) error {
	return r.db.Where("org_name = ? AND project_name = ? AND agent_name = ?", orgName, projectName, agentName).
		Delete(&models.AgentThunderClient{}).Error
}
