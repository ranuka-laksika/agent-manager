...
// AMS uses to reach an environment's Thunder, keyed by (OrgName, EnvName).
type EnvThunderSystemClient struct {
	ID                    uuid.UUID `gorm:"column:id;primaryKey;type:uuid;default:gen_random_uuid()"`
	OrgName               string    `gorm:"column:org_name;not null;uniqueIndex:uq_env_thunder_system_clients"`
