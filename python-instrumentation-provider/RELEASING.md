...
| **Pinned dependency locks** (frozen full tree per `traceloop × python`) | `python-instrumentation-provider/locks/traceloop-<T>-python<X.Y>.txt` — generated by `make lock` from `requirements.in` + `traceloop-sdk==<T>`; the image installs these directly (no live resolution). The **Instrumentation Provider Lock Check** CI job fails if they drift from `release-config.json` |
