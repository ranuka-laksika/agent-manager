...
2. Fill in the environment details.
   ...
   | **Isolation Tier** | Container runtime isolation for agents deployed to this environment — `runc` (default), `gVisor`, or `Kata Containers`. Stronger tiers need a dedicated node set up first; see [Isolation tiers](#isolation-tiers) below. | `runc (default)` |
...
Every environment has an **isolation tier** that decides which container runtime its agents run under. The tier is chosen at environment creation time and cannot be changed afterwards — it is shown as a chip in the environment list and on each agent's Deploy page.

| Tier | Runtime | Isolation | Requirements |
|------|---------|-----------|--------------|
| **runc** (default) | Standard container runtime | Sandboxed pod, shared host kernel | None — works everywhere |
| **gvisor** | gVisor (`runsc`) | Userspace kernel intercepts syscalls | A dedicated x86_64 node — see the [gVisor guide](./isolation-tiers/gvisor.mdx) |
| **kata** | Kata Containers (`kata-qemu`) | Per-agent VM with its own guest kernel | A dedicated node with KVM — see the [Kata guide](./isolation-tiers/kata.mdx) |
