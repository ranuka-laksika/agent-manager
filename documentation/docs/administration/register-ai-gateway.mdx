...
A gateway running **outside** the Agent Manager cluster connects to the gateway-management (xDS) endpoint on port `9243`. On the **quick-start** install this works out of the box (the installer exposes the endpoint and maps host port `9243`). On other Kubernetes installs the endpoint is a `ClusterIP` service by default, so expose it before registering an external gateway:
