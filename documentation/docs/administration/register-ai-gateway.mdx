...
The setup commands rendered by this page are pre-configured with the gateway control-plane URL for your installation, so no extra networking setup is needed here. Gateways deployed inside the Agent Manager cluster connect via cluster DNS; external gateways connect through the OpenChoreo control-plane gateway. If you installed Agent Manager on your own Kubernetes cluster, register a public hostname for the endpoint first — see the *External AI gateways* note in the [installation guide](../getting-started/on-your-environment.mdx).
