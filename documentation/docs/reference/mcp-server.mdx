...
WSO2 Agent Manager (AMP) ships a Model Context Protocol (MCP) server that lets MCP-capable AI assistants — Claude Code, Codex, Cursor, and others — drive the platform directly from the developer's terminal. Through the server, an assistant can list projects, register agents, trigger builds, deploy images, and inspect runtime traces without leaving the conversation.
...
The MCP server provides **16 tools** organized into five toolsets covering the agent lifecycle — from creating a project through observing a deployed agent's traces.
...
<details>
<summary><strong>Observability Toolset (4 tools)</strong></summary>
