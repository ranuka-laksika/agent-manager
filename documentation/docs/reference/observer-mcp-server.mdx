...
```json
{ "observerBaseUrl": "http://observer.amp.localhost:8080" }
```

Append `/mcp` to the returned `observerBaseUrl` to get the full MCP server URL, e.g.:

```
http://observer.amp.localhost:8080/mcp
```
...
The observer MCP server is authenticated the same way as the [main MCP server](./mcp-server.mdx#default-oauth-client) — the same pre-registered `am-mcp` OAuth client and pinned callback port `33418` apply, since both servers accept the same bearer token. Follow the [Configuring AI Assistants](./mcp-server.mdx#configuring-ai-assistants) instructions there, substituting the observer MCP server URL for the `agent-manager-service` one, for example:

```bash
claude mcp add --transport http agent-manager-observer http://observer.amp.localhost:8080/mcp \
  --client-id am-mcp \
  --callback-port 33418
```
