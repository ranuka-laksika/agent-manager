...
`amctl` ships with each Agent Manager release and shares its version. The commands below use **`0.17.0`** as an example — replace it with the latest version from the [releases page](https://github.com/wso2/agent-manager/releases).
...
```bash
curl -LO https://github.com/wso2/agent-manager/releases/download/amp/v0.17.0/amctl_v0.17.0_darwin_arm64.tar.gz
tar -xzf amctl_v0.17.0_darwin_arm64.tar.gz
...
```bash
curl -LO https://github.com/wso2/agent-manager/releases/download/amp/v0.17.0/amctl_v0.17.0_darwin_amd64.tar.gz
tar -xzf amctl_v0.17.0_darwin_amd64.tar.gz
sudo mv amctl /usr/local/bin/
```
...
```bash
curl -LO https://github.com/wso2/agent-manager/releases/download/amp/v0.17.0/amctl_v0.17.0_linux_amd64.tar.gz
tar -xzf amctl_v0.17.0_linux_amd64.tar.gz
sudo mv amctl /usr/local/bin/
```
...
```bash
curl -LO https://github.com/wso2/agent-manager/releases/download/amp/v0.17.0/amctl_v0.17.0_linux_arm64.tar.gz
tar -xzf amctl_v0.17.0_linux_arm64.tar.gz
sudo mv amctl /usr/local/bin/
```
...
```powershell
Invoke-WebRequest -Uri https://github.com/wso2/agent-manager/releases/download/amp/v0.17.0/amctl_v0.17.0_windows_amd64.zip -OutFile amctl.zip
