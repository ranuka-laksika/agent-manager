...
- **`403 insufficient permissions` on API calls.** You are signed in as Thunder's system `admin` account, which has no Agent Manager application role. Sign out and sign back in as `admin` (see [Log in](#log-in)).
...
- **`403 insufficient permissions` on API calls.** Sign in as `admin`, not Thunder's system `admin` account.
