...
The following host ports must be free before installation. The Agent Manager services do not need host ports of their own: the Console and API are served through the Control Plane gateway (8080), the Traces Observer through the Observability Plane gateway (11080), and trace ingestion through the Data Plane gateway (19080).

| Port | Purpose |
|------|---------|
| 6550 | Kubernetes API |
| 8080 | Control Plane Gateway (HTTP) — Console, API, Thunder, OpenChoreo API |
| 8443 | Control Plane Gateway (HTTPS) |
| 9243 | Gateway control plane — external AI gateways (requires `agentManagerService.gatewayMgtService.type=LoadBalancer`; the quick-start installer sets this) |
...
| **Traces Observer** | [http://traces.amp.localhost:11080](http://traces.amp.localhost:11080) |
